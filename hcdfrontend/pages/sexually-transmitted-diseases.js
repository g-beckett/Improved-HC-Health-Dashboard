...
const covid = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('https://hcdbackend.fly.dev/dataportal/_query');
          ...
          const covidDiseases = Diseases.filter(report => report.diseaseCategory === 'Sexually Transmitted Disease');
          const covidCaseReports = CaseReports.filter(report => report.DiseaseCategory === 'Sexually Transmitted Disease');
          const covidHospitalizedReports = HospitalizedReports.filter(report => report.DiseaseCategory === 'Sexually Transmitted Disease');
          const covidDeathReports = DeathReports.filter(report => report.DiseaseCategory === 'Sexually Transmitted Disease');
  ...
  return (
    ...
    {diseases ? (
      <p className="text-3xl font-semibold mb-4">Sexually Transmitted Disease Data for {today.split(' ')[0]}</p> 
       ) : ( 
      ...
      <div className="bg-gray-200 p-4 rounded mt-8">
        <h3 className="text-xl font-semibold mb-4">Sexually Transmitted Disease Hospitalization Data</h3>
