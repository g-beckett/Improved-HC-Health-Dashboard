      ...
      <div className="bg-gray-200 p-4 rounded mt-8">
        <ResponsiveContainer width="100%" height={500}>
          <LineChart
            ...
            margin={{ top: 20, right: 50, left: 20, bottom: 20 }} // Adjusted margins
            scrollable={true}
          >
            <XAxis
              dataKey="name"
              tick={{ textAnchor: 'middle', fontSize: 14 }}
              tickCount={12} // Adjusted tick count
              tickFormatter={(month) => new Date(2022, month - 1).toLocaleString('en-US', { month: 'long' })}
            />
            <YAxis tickFormatter={(tick) => tick.toLocaleString()} domain={['dataMin', 'dataMax']} /> {/* Adjusted Y-axis domain */}
